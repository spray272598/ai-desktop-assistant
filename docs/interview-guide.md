...
> 我做了一个本地优先的 **AI 桌面助手**。采用 DDD 分层的 Go 服务：  
> 意图识别（规则+LLM）→ 多 Agent 路由规划 → ReAct 循环调用工具。  
> 工具包括文件/命令、浏览器自动化、代码沙箱和 MCP 扩展。  
> 安全上有权限确认门与危险命令正则拦截；工程上有 MySQL 持久化、Redis 限流、SSE/WebSocket。  
> 前端是 React 控制台。我重点打磨了上下文管理、工具协议和可扩展架构。
